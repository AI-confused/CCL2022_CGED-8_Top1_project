...
class BertBiLstmCrfForNer(nn.Module):
    def __init__(self, model_name, config, dropout, rnn_layers):
        super(BertBiLstmCrfForNer, self).__init__()
        self.bert = BertModel.from_pretrained(model_name, config)
        self.hidden_dim = config.hidden_size
        self.lstm = nn.LSTM(config.hidden_size, config.hidden_size, num_layers=rnn_layers, bidirectional=True, dropout=dropout, batch_first=True)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        lstm_out, hidden = self.lstm(sequence_output)
        # lstm_out = lstm_out.contiguous().view(-1, self.hidden_dim*2)
        # d_lstm_out = self.dropout(lstm_out)

        l_out = self.classifier(lstm_out)
        # outputs = (l_out,)

        if labels is not None:
            loss = self.crf(emissions = l_out, tags=labels, mask=attention_mask)
            # outputs =(-1*loss,)+outputs
            outputs = -1 * loss
        else:
            outputs = self.crf.decode(l_out, attention_mask).squeeze(0)
        return outputs # (loss), scores


class BertCrfForNer(nn.Module):
    def __init__(self, model_name, config, dropout):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None:
            ...
            outputs = self.crf.decode(logits, attention_mask).squeeze(0)
        return outputs # (loss), scores
