...
if __name__ == '__main__':
    # init task utils
    # task_utils = BaseUtils(task_config_path=os.path.join(os.getcwd(), 'config/bert_bilstm_crf_train_pretrain.yml'))
    task_utils = BaseUtils(task_config_path=os.path.join(os.getcwd(), 'config/bert_bilstm_crf_train_finetune.yml'))
    ...
    if not task_setting.skip_train:
        ...
        # 训练集为历年数据集，则基于lang8的模型继续训练
            ...
            task.train(resume_model_path='$lang8预训练最佳模型')
    # do test
    else:
